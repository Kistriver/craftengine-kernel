...
run: docker
	docker run --name ce-kernel-alpha -ti --rm -e REDIS_PORT=6379 -e REDIS_DB=1 -e CE_PROJECT_NAME="CRAFTEngine" \
...
cluster: docker
	...
	docker run --name ce-kernel-alpha -d -e REDIS_PORT=6379 -e REDIS_DB=1 -e CE_PROJECT_NAME="CRAFTEngine" \
	-e CE_NODE_NAME="alpha" --link ce-redis:redis -p 2011:2011 \
	-v /var/run/docker.sock:/var/run/docker.sock --privileged kistriver/ce-kernel
